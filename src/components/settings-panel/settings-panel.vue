    ...
    <v-bottom-sheet v-model="sheet">
      <v-sheet class="text-center" height="250px">
        ...
        <div class="input-wrapper">
            <v-text-field v-model="ipAddress" clearable placeholder="192.168.1.23" label="Sony TV IP-address"></v-text-field>
        </div>
       <v-btn @click="ping">
        Testa anslutning
    </v-btn>
     <v-icon x-large class="ping-status" :color="validConnection ? 'green' : 'red'">mdi-check</v-icon>
      </v-sheet>
    </v-bottom-sheet>
  </div>
</template>
